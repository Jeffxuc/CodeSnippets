...
public:
    ...
    {
        return CM_Enable_DevNode(m_pDevInfoList[index].DevInst, 0);
    ...
    {
        return CM_Disable_DevNode(m_pDevInfoList[index].DevInst, 0);
    ...
    DWORD GetProblemCode(IN int index, OUT unsigned long& problemCode)
    {
        return this->GetPropertyDWORD(index, &DEVPKEY_Device_ProblemCode, problemCode);
    }

	DWORD GetDriverDate(IN int index, OUT wstring& date)
	{
        FILETIME time = {0};
        this->GetPropertyFileTime(index, &DEVPKEY_Device_DriverDate, time);
        SYSTEMTIME sysTime;
        FileTimeToSystemTime(&time, &sysTime);
        wstring year_str = std::to_wstring(sysTime.wYear);
        wstring month_str = std::to_wstring(sysTime.wMonth);
        wstring day_str = std::to_wstring(sysTime.wDay);
        wstring hour_str = std::to_wstring(sysTime.wHour);
        wstring minute_str = std::to_wstring(sysTime.wMinute);
        wstring second_str = std::to_wstring(sysTime.wSecond);
        return 0;

		// return this->GetDate(index, Date);
	}

	DWORD GetDriverVersion(IN int index, OUT unsigned int& Version)
	{
		return this->GetVersion(index, Version);
	}

    DWORD GetDriverData_01(IN int index, IN const DEVPROPKEY* propertyKey, OUT FILETIME& timeProperty)
    {
        return this->GetPropertyFileTime_01(index, propertyKey, timeProperty);
    }

    DWORD GetDriverVersion_01(IN int index, IN const DEVPROPKEY* propertyKey, OUT wstring& versionProperty)
    {
        return this->GetPropertyStr(index, propertyKey, versionProperty);
    }
...
private:
    ...
    /// 属性键
    /// </PARAM>
    /// <PARAM name = "dwProperty" dir = "OUT">
    /// 保存设备属性
    /// </PARAM>
    /// <RETURNS>
    /// 成功返回0 失败返回ErrorCode
    /// </RETURNS>
    DWORD GetPropertyDWORD(IN int index, IN const DEVPROPKEY* propertyKey, OUT DWORD& dwProperty)
    {
        DWORD bufferSize = 0;
        PBYTE pBuffer = NULL;
    ...
    DWORD GetPropertyFileTime_01(IN int index, IN const DEVPROPKEY* propertyKey, OUT FILETIME& timeProperty)
    {
        DWORD bufferSize = sizeof(FILETIME);
        //使用指针时需要正确分配对应的类型，不能直接使用NULL，否则会报错
        PFILETIME pBuffer = (PFILETIME)LocalAlloc(LPTR, bufferSize);
        DEVPROPTYPE dataType;
        ...
        while (true)
        {
            ...
            else
            {
                if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
                {
                    ...
                    pBuffer = (PFILETIME)LocalAlloc(LPTR, bufferSize);
                }
                else
                {
                    returnCode = GetLastError();
    ...
    /// 成功返回0 失败返回ErrorCode
    /// </RETURNS>
    DWORD GetPropertyFileTime(IN int index, IN const DEVPROPKEY* propertyKey, OUT FILETIME& timeProperty)
    {
        DWORD bufferSize = sizeof(FILETIME);
        DEVPROPTYPE dataType;
        FILETIME fileTime = { 0 };
	...
	DWORD GetDate(IN int index, OUT wstring& Date)
	{

		BOOL bRet1 = SetupDiBuildDriverInfoList(m_hDevInfoSet, &m_pDevInfoList[index], SPDIT_COMPATDRIVER);
		if (FALSE == bRet1)
			return GetLastError();
		//遍历驱动
		...
		int j = 0;
        for (; SetupDiEnumDriverInfo(m_hDevInfoSet, &m_pDevInfoList[index], SPDIT_COMPATDRIVER, j, &driverInfoData); ++j)
        {
            break;
        }
		...
		Date = year_str + L"-" + month_str + L"-" + day_str + L" " + hour_str + L"-" + minute_str + L"-" + second_str;

		return 0;
	}

	DWORD GetVersion(IN int index, OUT unsigned int& Version)
	{
		BOOL bRet1 = SetupDiBuildDriverInfoList(m_hDevInfoSet, &m_pDevInfoList[index], SPDIT_COMPATDRIVER);
		if (FALSE == bRet1)
			return GetLastError();
	    ...
	    Version = (unsigned int)driverInfoData.DriverVersion;	
		return 0;
	}

    /* 不是universal API 暂时注释
    /// <SUMMARY>
    /// 改变设备状态
    /// 该方法需要管理员权限
    /// </SUMMARY>
...
DWORD LSetupDev::GetProblemCode(IN int index, OUT unsigned long& problemCode)
{
    return m_pSADevObject->GetProblemCode(index, problemCode);
}

DWORD LSetupDev::GetDriverDate(IN int index, OUT wstring& Date)
{
	return m_pSADevObject->GetDriverDate(index, Date);
}

DWORD LSetupDev::GetDriverVersion(IN int index, OUT unsigned int& Version)
{
	return m_pSADevObject->GetDriverVersion(index, Version);
}

DWORD LSetupDev::GetDriverData_01(IN int index, OUT wstring& date)
{
    FILETIME driverDate;
    DWORD retCode = m_pSADevObject->GetDriverData_01(index, &DEVPKEY_Device_DriverDate, driverDate);

    SYSTEMTIME sysTime;
    FileTimeToSystemTime(&driverDate, &sysTime);
    wstring wsYear = std::to_wstring(sysTime.wYear);
    wstring wsMonth = std::to_wstring(sysTime.wMonth);
    wstring wsDay = std::to_wstring(sysTime.wDay);
    
    date = wsYear + L"-" + wsMonth + L"-" + wsDay;

    return retCode;
}

DWORD LSetupDev::GetDriverVersion_01(IN int index, OUT wstring& version)
{
    m_pSADevObject->GetDriverVersion_01(index, &DEVPKEY_Device_DriverVersion, version);
    return 0;
}
...
LSetupMonitor::LSetupMonitor(): LSetupDev()
{
...
LSetupBluetooth::LSetupBluetooth()
    : LSetupDev()
{
    m_pSADevObject->Scan(&GUID_DEVCLASS_BLUETOOTH);
}
...
LSetupNet::LSetupNet()
{
    m_pSADevObject->Scan(&GUID_DEVCLASS_NET);
}
...
LSetupBattery::LSetupBattery()
{
    m_pSADevObject->Scan(&GUID_DEVCLASS_BATTERY);
}
...
LSetupUSB::LSetupUSB()
{
	m_pSADevObject->Scan(&GUID_DEVINTERFACE_USB_DEVICE);
}
